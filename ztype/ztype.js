        ...
        drawGraph: function(x, y) {
            ...
            for (var i = 0; i < max; i++) {
                ...
                if (lastp) {
                    ...
                    ctx.lineTo(rx + 1.5, ry + 1.5) ctx.stroke();
                }
                lastp = {
                    x: rx,
                    y: ry
    ...
    DetailedStats = ig.Class.extend({
        ...
        drawOnCanvas: function(canvas, mx, my) {
            ...
            for (var i = 0; i < numLabels; i++) {
                ...
                ctx.fillStyle = '#555';
                ctx.fillText(label, xp, yp + 3) ctx.beginPath();
                ctx.moveTo(x, yp);
                ctx.lineTo(x + rw, yp);
                ctx.stroke();
            }
            ...
            if (plot) {
                ...
                ctx.fillStyle = '#aaa';
                ctx.font = '10px sans-serif'ctx.fillText(date, bx + 16, by + 18);
                var scdesc = 'SCORE ' + plot.game.score;
                ctx.fillStyle = '#555';
                ctx.font = '12px sans-serif'ctx.fillText(scdesc, bx + 16, by + 36);
                var desc = 'WAVE ' + plot.game.wave + ' / ACCURACY ' + plot.game.accuracy.toFixed(2) + '%' + ' / STREAK ' + plot.game.streak;
                ctx.fillStyle = '#555';
                ctx.font = '12px sans-serif'ctx.fillText(desc, bx + 16, by + 52);
            }
        },
